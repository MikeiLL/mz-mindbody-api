...
final class WP_Session extends Recursive_ArrayAccess {
    ...
    public function write_data() {
        ...
        if (defined('WP_SESSION_USE_OPTIONS') && WP_SESSION_USE_OPTIONS) {
            ...
            if ( false === WP_Session_Utils::session_exists( $this->session_id ) ) {
                WP_Session_Utils::add_session( array( 'session_key' => $this->session_id, 'session_value' => serialize($this->container), 'session_expiry' => $this->expires ) );
            } else {
                WP_Session_Utils::update_session( $this->session_id, array( 'session_value' => serialize($this->container) ) );
            }
        }
    }
    /**
