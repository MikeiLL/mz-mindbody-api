...
/******/ 	return __webpack_require__(__webpack_require__.s = "./scripts/jquery.filtertable.mz.js");
/******/ })
/************************************************************************/
/******/ ({
...
(function ($) {
  ...
  if (jmajor < 2 && jminor < 8) {
    ...
    $.expr[':'].filterTableFind = function (a, i, m) {
      ...
      if (window.mz_mbo_selectValue == '0' || $(el).find('.mz_location_' + window.mz_mbo_selectValue).length > 0) return $(a).text().toUpperCase().indexOf(m[3].toUpperCase()) >= 0;
    };
  } else {
    // build the pseudo selector for jQuery >= 1.8
  ...
  $.fn.filterTable = function (options) {
    ...
    var defaults = {
      ...
      showAllHeaderRows: true,
      // if true, table rows with class header will always show
      highlightClass: 'alt',
      ...
      minRows: 8,
      // don't show the filter on tables with less than this number of rows
      ...
      // label in default select option
      locations: {} // empty array to hold locations

    },
        hsc = function hsc(text) {
    ...
    var doFiltering = function doFiltering(table, q) {
    ...
    return this.each(function () {
      ...
      if (locations.lengh >= 2) {
        ...
        selector.bind('change', function () {
          // bind doFiltering() to click and search events
...
/******/ });
//# sourceMappingURL=jquery.filtertable.mz.js.map