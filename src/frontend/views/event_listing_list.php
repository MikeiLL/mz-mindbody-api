...
if ((is_array($data->events)) && !empty($data->events)) : ?>
    ...
    <?php foreach ($data->events as $date => $events) : ?>
        ...
        <tbody>
            <tr>
                <td><?php echo date_i18n(Core\MzMindbody_Api::$date_format, strtotime($event->startDateTime)); ?> <br /><?php echo $event->sign_up_link->build(); ?></td>
                <td>
                    <?php echo date_i18n(Core\MzMindbody_Api::$time_format, strtotime($event->startDateTime)); ?> -
                    <?php echo date_i18n(Core\MzMindbody_Api::$time_format, strtotime($event->endDateTime)); ?>
                </td>
                <td>
