...
class MZ_Events {
	...
	public function mZ_mindbody_show_events ($atts, $account=0) {
		...
		*/
		...
		if (!empty($mz_sessions) && ($mz_sessions[0] != 0))
		{
			...
			if(!empty($mz_event_data['GetClassesResult']['Classes']['Class']))
			{
				...
				if ($number_of_events >= 1)
				{
					...
					$return .= Navigation::render_schedule_navigation($mz_date, "Events", MZ_MBO_shared::$mz_event_calendar_duration);
					if ($list_only != 1):
		...
		$return .= Navigation::render_schedule_navigation($mz_date, _n("Event", "Events", 'mz-mindbody-api'), MZ_MBO_shared::$mz_event_calendar_duration);
		return $return;
