...
class ControlStructureSpacingSniff extends Sniff
{
    ...
    public function process_token($stackPtr)
    {
        ...
        if (
            isset($this->tokens[ ( $stackPtr + 1 ) ]) && \T_WHITESPACE !== $this->tokens[ ( $stackPtr + 1 ) ]['code']
        ...
        if (false !== $this->blank_line_check && isset($scopeOpener)) {
            ...
            if (
                ! isset($ignore[ $this->tokens[ $firstContent ]['code'] ])
            ...
            if ($firstContent !== $scopeCloser) {
                ...
                if (
                    ! isset($ignore[ $this->tokens[ $checkToken ]['code'] ])
                    && $this->tokens[ $lastContent ]['line'] <= ( $this->tokens[ $scopeCloser ]['line'] - 2 )
                ) {
                    for ($i = ( $scopeCloser - 1 ); $i > $lastContent; $i--) {
                        if (
                            $this->tokens[ $i ]['line'] < $this->tokens[ $scopeCloser ]['line']
                            ...
                            if (true === $fix) {
                                 ...
                                 */
                                if (
                                    \T_COMMENT !== $this->tokens[ $lastContent ]['code']
        ...
        if (
            \T_COMMENT === $this->tokens[ $trailingContent ]['code']
        ...
        if (
            isset($this->tokens[ $trailingContent ]['scope_condition'])
            ...
            if (( $this->tokens[ $scopeCloser ]['line'] + 1 ) !== $this->tokens[ $trailingContent ]['line']) {
                ...
                if (true === $fix) {
                    ...
                    if (
                        \T_COMMENT !== $this->tokens[ $scopeCloser ]['code']
